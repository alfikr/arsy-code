...
| `plan.*` | **PARTIAL** | A structured plan survives registry rebuilds and transcript trimming within the process. State is held in a process-global map and is lost on restart. Despite earlier comments calling it durable, it is scratch planning at present. |
...
| `validate.*` | **PARTIAL** | `validate.record` verifies that a cited `ProcessResult` artifact ran the named command and exited successfully. Its log is process-local; records omit the evidence ID, task, criterion, and workspace revision. `validate.status` only describes the last check. |
| `TaskGraph` | **PARTIAL** | Durable nodes, dependencies, state transitions, leases, cycle detection, join predicates, budgets, and attenuated child grants exist. Parent-child identity, attempt records, typed results, durable worker descriptors, aggregate budget reservation/settlement, and active scheduler ownership do not. |
...
| Restart recovery | **PARTIAL** | Expired/running task leases can be reclaimed. Resume selects pending work and rebuilds execution from current workspace policy/model; it does not reconstruct a child from a persisted role, original authority, model, workspace view, and attempt descriptor. |
