...
#[cfg(feature = "tui")]
fn auth_remove_handle(_invocation: &Invocation, answer: &str) -> Result<AuthNext, String> {
    ...
    // The catalog is file-only now; a handle naming the withdrawn keyring is
    // removed from the listing, and the secret behind it is unreachable.
    let _ = FileCredentialStore.remove(handle.name());
